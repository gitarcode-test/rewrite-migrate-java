...
public class LombokValueToRecord extends ScanningRecipe<Map<String, Set<String>>> {
  ...
  private static final AnnotationMatcher LOMBOK_VALUE_MATCHER =
      new AnnotationMatcher("@lombok.Value()");
  private static final AnnotationMatcher LOMBOK_BUILDER_MATCHER =
      new AnnotationMatcher("@lombok.Builder()");

  @Option(
      displayName = "Add a `toString()` implementation matching Lombok",
      description = "When set the `toString` format from Lombok is used in the migrated record.",
      required = false)
  @Nullable Boolean useExactToString;
  ...
  public TreeVisitor<?, ExecutionContext> getScanner(Map<String, Set<String>> acc) {
    TreeVisitor<?, ExecutionContext> check =
        Preconditions.and(new UsesJavaVersion<>(17), new UsesType<>("lombok.Value", false));
    return Preconditions.check(check, new ScannerVisitor(acc));
  }

  @Override
  public TreeVisitor<?, ExecutionContext> getVisitor(
      Map<String, Set<String>> recordTypesToMembers) {
    return new LombokValueToRecord.LombokValueToRecordVisitor(
        useExactToString, recordTypesToMembers);
  }
  ...
  private static class ScannerVisitor extends JavaIsoVisitor<ExecutionContext> {
    ...
    public J.ClassDeclaration visitClassDeclaration(
      ...
      acc.putIfAbsent(cd.getType().getFullyQualifiedName(), memberVariableNames);
    ...
    private boolean isRelevantClass(J.ClassDeclaration classDeclaration) {
      ...
      return classDeclaration.getType() != null
          ...
          && classDeclaration.getBody().getStatements().stream()
              .allMatch(this::isRecordCompatibleField)
          && !hasIncompatibleModifier(classDeclaration);
    }

    private static Predicate<J.Annotation> matchAnnotationWithNoArguments(
        AnnotationMatcher matcher) {
      return ann ->
          matcher.matches(ann) && (ann.getArguments() == null || ann.getArguments().isEmpty());
    }
    ...
    /**
     * If the class target class implements an interface, transforming it to a record will not work
     * in general, because the record access methods do not have the "get" prefix.
     *
     * @param classDeclaration
     * @return true if the class implements an interface with a getter method based on a member
     *     variable
     */
    private boolean implementsConflictingInterfaces(
        J.ClassDeclaration classDeclaration, Set<String> memberVariableNames) {
      List<TypeTree> classDeclarationImplements = classDeclaration.getImplements();
      if (classDeclarationImplements == null) {
        return false;
      }
      return classDeclarationImplements.stream()
          .anyMatch(
              implemented -> {
                JavaType type = implemented.getType();
                if (type instanceof JavaType.FullyQualified) {
                  return isConflictingInterface(
                      (JavaType.FullyQualified) type, memberVariableNames);
                } else {
                  return false;
                }
              });
    ...
    private static boolean isConflictingInterface(
        JavaType.FullyQualified implemented, Set<String> memberVariableNames) {
      boolean hasConflictingMethod =
          implemented.getMethods().stream()
      ...
      if (superInterfaces != null) {
        return superInterfaces.stream()
            .anyMatch(i -> isConflictingInterface(i, memberVariableNames));
      }
      return false;
    }
    ...
    private boolean hasIncompatibleModifier(J.ClassDeclaration classDeclaration) {
      ...
      if (getCursor().getParent() != null) {
        ...
        if (parentValue instanceof J.ClassDeclaration
            || (parentValue instanceof JRightPadded
                && ((JRightPadded) parentValue).getElement() instanceof J.ClassDeclaration)) {
          if (classDeclaration.getModifiers().stream()
              .noneMatch(mod -> mod.getType() == J.Modifier.Type.Static)) {
            return true;
          }
        }
      }
  ...
  private static class LombokValueToRecordVisitor extends JavaIsoVisitor<ExecutionContext> {
    ...
    public LombokValueToRecordVisitor(
        @Nullable Boolean useExactToString, Map<String, Set<String>> recordTypeToMembers) {
    ...
    private static String getterMethodNameToFluentMethodName(String methodName) {
      StringBuilder fluentMethodName =
          new StringBuilder(methodName.replace(STANDARD_GETTER_PREFIX, ""));
    ...
    private static List<Statement> mapToConstructorArguments(
      ...
      return memberVariables.stream()
          .map(it -> it.withModifiers(Collections.emptyList()).withVariables(it.getVariables()))
          .map(Statement.class::cast)
          .collect(toList());
    }

    private J.ClassDeclaration addExactToStringMethod(
        J.ClassDeclaration classDeclaration, List<J.VariableDeclarations> memberVariables) {
      return classDeclaration.withBody(
          TO_STRING_TEMPLATE.apply(
              new Cursor(getCursor(), classDeclaration.getBody()),
    ...
    private static JavaType.Class buildRecordType(J.ClassDeclaration classDeclaration) {
      ...
      return JavaType.ShallowClass.build(className).withKind(JavaType.FullyQualified.Kind.Record);
    }
    ...
    public J.ClassDeclaration visitClassDeclaration(J.ClassDeclaration cd, ExecutionContext ctx) {
      ...
      if (classType == null
          || !recordTypeToMembers.containsKey(classType.getFullyQualifiedName())) {
        return classDeclaration;
      }

      List<J.VariableDeclarations> memberVariables =
          findAllClassFields(classDeclaration).collect(toList());
      ...
      classDeclaration =
          new RemoveAnnotationVisitor(LOMBOK_VALUE_MATCHER)
      ...
      classDeclaration =
          classDeclaration
              ...
              .withBody(classDeclaration.getBody().withStatements(bodyStatements))
