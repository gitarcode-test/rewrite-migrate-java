...
public class JpaCacheProperties extends Recipe {
  ...
  public TreeVisitor<?, ExecutionContext> getVisitor() {
    return Preconditions.check(
        new FindSourceFiles("**/persistence.xml"), new PersistenceXmlVisitor());
  }
}
...
class PersistenceXmlVisitor extends XmlVisitor<ExecutionContext> {
  ...
  public Xml visitTag(Xml.Tag tag, ExecutionContext ctx) {
    ...
    if (sdh.sharedCacheModeElement != null || sdh.sharedCacheModeProperty != null) {
      ...
      if (sdh.sharedCacheModeElement != null && sdh.sharedCacheModeElementUnspecified) {
        ...
        if (sdh.sharedCacheModeProperty != null && sdh.sharedCacheModePropertyUnspecified) {
          ...
          Xml.Tag updatedProp =
              updateAttributeValue("value", scmValue, sdh.sharedCacheModeProperty);
          //noinspection unchecked
          sdh.propertiesElement =
              sdh.propertiesElement.withContent(
                  ListUtils.map(
                      (List<Content>) sdh.propertiesElement.getContent(),
                      content -> content == sdh.sharedCacheModeProperty ? updatedProp : content));
      ...
      if (scmValue != null) {
        if (!v1) {
          Xml.Tag newNode =
              Xml.Tag.build("<shared-cache-mode>" + scmValue + "</shared-cache-mode>");
          // Ideally we would insert <shared-cache-mode> before the <validation-mode> and
          // <properties> nodes
          ...
          if (eclipseLinkPropValue != null) {
            ...
            Xml.Tag newElement =
                Xml.Tag.build(
                    "<property name=\"eclipselink.cache.shared.default\" value=\""
                        + eclipseLinkPropValue
                        + "\"></property>");

            sdh.propertiesElement =
                addOrUpdateChild(sdh.propertiesElement, newElement, getCursor().getParentOrThrow());
    ...
    if (sdh.openJPACacheProperty != null) {
      ...
      if ("true".equalsIgnoreCase(attrValue) || "false".equalsIgnoreCase(attrValue)) {
        sdh.propertiesElement =
            filterTagChildren(sdh.propertiesElement, child -> child != sdh.openJPACacheProperty);
        t = addOrUpdateChild(t, sdh.propertiesElement, getCursor().getParentOrThrow());
      }
    }
    ...
    if (sdh.sharedCacheModeElement != null && sdh.sharedCacheModeProperty != null) {
      sdh.propertiesElement =
          filterTagChildren(sdh.propertiesElement, child -> child != sdh.sharedCacheModeProperty);
      t = addOrUpdateChild(t, sdh.propertiesElement, getCursor().getParentOrThrow());
    }
    return t;
  }
  ...
  private SharedDataHolder extractData(Xml.Tag puNode) {
    ...
    sdh.sharedCacheModeElementUnspecified =
        sdh.sharedCacheModeElement != null
            && SHARED_CACHE_MODE_VALUE_UNSPECIFIED.equals(
                getTextContent(sdh.sharedCacheModeElement));
    // true if shared-cache-mode set to UNSPECIFIED.
    sdh.sharedCacheModePropertyUnspecified =
        sdh.sharedCacheModeProperty != null
            && SHARED_CACHE_MODE_VALUE_UNSPECIFIED.equals(
                getAttributeValue("value", sdh.sharedCacheModeProperty));
