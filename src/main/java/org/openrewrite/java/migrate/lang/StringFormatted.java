...
public class StringFormatted extends Recipe {
  ...
  public String getDescription() {
    return "Prefer `String.formatted(Object...)` over `String.format(String, Object...)` in Java 17"
               + " or higher.";
  }
  ...
  private static class StringFormattedVisitor extends JavaVisitor<ExecutionContext> {
    ...
    public J visitMethodInvocation(J.MethodInvocation m, ExecutionContext ctx) {
      ...
      JavaType.Method formatted =
          m.getMethodType().getDeclaringType().getMethods().stream()
      ...
      Expression select =
          wrapperNotNeeded
              ? arguments.get(0)
              : new J.Parentheses<>(
                  randomId(), Space.EMPTY, Markers.EMPTY, JRightPadded.build(arguments.get(0)));
