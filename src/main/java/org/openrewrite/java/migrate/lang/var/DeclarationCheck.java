...
final class DeclarationCheck {
   ...
   *
   * <p>Var is applicable inside methods and initializer blocks for single variable definition. Var
   * is *not* applicable to method definitions.
   *
  ...
  public static boolean isVarApplicable(Cursor cursor, J.VariableDeclarations vd) {
    if (isField(vd, cursor)
        || isMethodParameter(vd, cursor)
        || !isSingleVariableDefinition(vd)
        || initializedByTernary(vd)) {
      return false;
    }
  ...
  /**
   * Determine if a variable definition defines a single variable that is directly initialized with
   * value different from null, which not make use of var.
   *
  ...
  private static boolean isSingleVariableDefinition(J.VariableDeclarations vd) {
    ...
    boolean isNullAssigment =
        initializer instanceof J.Literal && ((J.Literal) initializer).getValue() == null;
    boolean alreadyUseVar =
        typeExpression instanceof J.Identifier
  ...
  /**
   * Determines if a cursor is contained inside a Method declaration without an intermediate Class
   * declaration
   *
  ...
  private static boolean isInsideMethod(Cursor cursor) {
    Object value =
        cursor
            .dropParentUntil(
                p ->
                    p instanceof J.MethodDeclaration
                        || p instanceof J.ClassDeclaration
                        || p.equals(Cursor.ROOT_VALUE))
            .getValue();
  ...
  private static boolean isField(J.VariableDeclarations vd, Cursor cursor) {
    ...
    return parent.getValue() instanceof J.Block
        && (grandparent.getValue() instanceof J.ClassDeclaration
            || grandparent.getValue() instanceof J.NewClass);
  }
  ...
  private static boolean isInsideInitializer(Cursor cursor, int nestedBlockLevel) {
    ...
    // initializer blocks are blocks inside the class definition block, therefor a nesting of 2 is
    // mandatory
