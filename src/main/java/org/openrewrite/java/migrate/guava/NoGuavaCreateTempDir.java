...
public class NoGuavaCreateTempDir extends Recipe {
  ...
  public String getDescription() {
    return "Replaces Guava `Files#createTempDir()` with Java `Files#createTempDirectory(..)`."
               + " Transformations are limited to scopes throwing or catching"
               + " `java.io.IOException`.";
  }
  ...
  public TreeVisitor<?, ExecutionContext> getVisitor() {
    return Preconditions.check(
        new UsesMethod<>("com.google.common.io.Files createTempDir()"),
        new NoGuavaTempDirVisitor());
  }

  private static class NoGuavaTempDirVisitor extends JavaIsoVisitor<ExecutionContext> {
    private static final MethodMatcher guavaCreateTempDirMatcher =
        new MethodMatcher("com.google.common.io.Files createTempDir()");
      ...
      if (guavaCreateTempDirMatcher.matches(mi)) {
                ...
                .dropParentUntil(
                    j ->
                        j instanceof J.MethodDeclaration
                            || j instanceof J.Try
                            || j instanceof J.ClassDeclaration);
        ...
        if (parentValue instanceof J.MethodDeclaration) {
          ...
          if (md.getThrows() != null
              && md.getThrows().stream()
                  .anyMatch(
                      n -> isIOExceptionOrException(TypeUtils.asFullyQualified(n.getType())))) {
            mi = toFilesCreateTempDir(mi);
          }
        } else if (parentValue instanceof J.Try) {
          J.Try tr = (J.Try) parentValue;
          if (tr.getCatches().stream()
              .anyMatch(
                  n ->
                      isIOExceptionOrException(
                          TypeUtils.asFullyQualified(n.getParameter().getTree().getType())))) {
            mi = toFilesCreateTempDir(mi);
          }
        }
      }
