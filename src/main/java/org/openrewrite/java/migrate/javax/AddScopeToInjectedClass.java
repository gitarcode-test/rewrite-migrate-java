...
public class AddScopeToInjectedClass extends ScanningRecipe<Set<String>> {
  ...
  private static final String JAVAX_ENTERPRISE_CONTEXT_DEPENDENT =
  ...
  public String getDescription() {
    return "Finds member variables annotated with `@Inject' and applies `@Dependent` scope"
               + " annotation to the variable's type.";
  }
      ...
      public J.CompilationUnit visitCompilationUnit(
        ...
        for (J.ClassDeclaration aClass : cu.getClasses()) {
          if (aClass.getType() != null
              && injectedTypes.contains(aClass.getType().getFullyQualifiedName())) {
            return (J.CompilationUnit)
                new AnnotateTypesVisitor(JAVAX_ENTERPRISE_CONTEXT_DEPENDENT)
