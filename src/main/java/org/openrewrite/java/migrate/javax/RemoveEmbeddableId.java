...
public class RemoveEmbeddableId extends ScanningRecipe<RemoveEmbeddableId.Accumulator> {
  ...
  public String getDisplayName() {
    return "`@Embeddable` classes cannot have an `@Id` annotation when referenced by an"
               + " `@EmbeddedId` annotation";
  }

  @Override
  public String getDescription() {
    return "According to the Java Persistence API (JPA) specification, if an entity defines an"
               + " attribute with an `@EmbeddedId` annotation, the embeddable class cannot contain"
               + " an attribute with an `@Id` annotation. If both the `@EmbeddedId` annotation and"
               + " the `@Id` annotation are defined, OpenJPA ignores the `@Id` annotation, whereas"
    ...
    return Preconditions.check(
        ...
        new JavaIsoVisitor<ExecutionContext>() {
          ...
          public J.ClassDeclaration visitClassDeclaration(
            ...
            // Ensure class has @Embeddable annotation, and was tagged with @EmbeddedId from another
            // class
            if (!FindAnnotations.find(classDecl, "@javax.persistence.Embeddable").isEmpty()
                && acc.isEmbeddableClass(classDecl.getType())) {
              // Remove @Id annotation from anything in the class (only found on
              // VariableDeclarations)
              classDecl =
                  new RemoveAnnotation("javax.persistence.Id")
                      .getVisitor()
